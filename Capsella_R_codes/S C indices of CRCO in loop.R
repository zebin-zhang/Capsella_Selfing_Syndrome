...
for (i in tissue) {
  ...
  S$Ave_CRCO <- rowMeans(S[,c("mean_CR","mean_CO")])
  ...
  S$Cbp_Cg <- (S$mean_Cbp_Cg - S$Ave_CRCO) / S$Ave_CRCO  # Subgenome Cg 
  S$ASI_Cg <- (S$mean_ASI_Cg - S$Ave_CRCO) / S$Ave_CRCO
  S$EUR_Cg <- (S$mean_EUR_Cg - S$Ave_CRCO) / S$Ave_CRCO
  S$ME_Cg <- (S$mean_ME_Cg - S$Ave_CRCO) / S$Ave_CRCO
  S$CASI_Cg <- (S$mean_CASI_Cg - S$Ave_CRCO) / S$Ave_CRCO
  
  S$Cbp_Co <- (S$mean_Cbp_Co - S$Ave_CRCO) / S$Ave_CRCO  # Subgenome Co
  S$ASI_Co <- (S$mean_ASI_Co - S$Ave_CRCO) / S$Ave_CRCO
  S$EUR_Co <- (S$mean_EUR_Co - S$Ave_CRCO) / S$Ave_CRCO
  S$ME_Co <- (S$mean_ME_Co - S$Ave_CRCO) / S$Ave_CRCO
  S$CASI_Co <- (S$mean_CASI_Co - S$Ave_CRCO) / S$Ave_CRCO
  ...
  for (j in population) {
    ...
    if(sub(".*_", "", j) == "Cg"){
      ...
      p1 <- ggplot(S, aes(x=gg)) + 
        ...
        annotate(geom="text", x=-1, y=1.9, label="Towards CR", hjust=0) +
  ...
  for (com in c("CGCO", "CGCR", "CRCO")){
    ...
    # keep genes with CRCO FC > 2 &FDR ≤ 0.05
    ...
    write.table(S, file = paste0("OutputData/CRCO_S_All_",Tissue,".txt"), quote = F, sep = "\t", row.names = F)
    
    assign(paste0("S_DE_",com,"_",i), S_DE)
    write.table(S_DE, file = paste0("OutputData/CRCO_S_",com,"_DEs_",Tissue,".txt"), quote = F, sep = "\t", row.names = F)
    ...
    S_DE$C_delta_Parents <- abs(S_DE$mean_CR - S_DE$mean_CO)
    ...
    S_DE$C_delta_Cbp_co <- abs(S_DE$mean_Cbp_Co - S_DE$mean_CR)
    S_DE$C_delta_ASI_cg <- abs(S_DE$mean_ASI_Cg - S_DE$mean_CO) # ASI
    S_DE$C_delta_ASI_co <- abs(S_DE$mean_ASI_Co - S_DE$mean_CR)
    S_DE$C_delta_EUR_cg <- abs(S_DE$mean_EUR_Cg - S_DE$mean_CO) # EUR
    S_DE$C_delta_EUR_co <- abs(S_DE$mean_EUR_Co - S_DE$mean_CR)
    S_DE$C_delta_ME_cg <- abs(S_DE$mean_ME_Cg - S_DE$mean_CO) # ME
    S_DE$C_delta_ME_co <- abs(S_DE$mean_ME_Co - S_DE$mean_CR)
    S_DE$C_delta_CASI_cg <- abs(S_DE$mean_CASI_Cg - S_DE$mean_CO) # CASI
    S_DE$C_delta_CASI_co <- abs(S_DE$mean_CASI_Co - S_DE$mean_CR)
    ...
    write.table(S_DE, file = paste0("OutputData/CRCO_SC_",com,"_DEs_",Tissue,".txt"), quote = F, sep = "\t", row.names = F)
...
CRCO_S_case <- rep(c(rep("All",3), rep("Phylogeny",3), rep("MST", 3), rep("Phylogeny + MST",3)),3) # 3 tissues
CRCO_S_index <- c(rep(c("delta_S","S_Co", "S_Cg"), 12)) # 4 cases, 3 tissues (F,L,R)
CRCO_S_tissue <- c(rep("Flower", 12), rep("Leaf", 12), rep("Root", 12))
# CRCO_S_value <- c(abs(median(S_F$Cbp_Co)) - abs(median(S_F$Cbp_Cg)), # Flower all
#                   median(S_F$Cbp_Co),
#                   median(S_F$Cbp_Cg),
#                   abs(median(S_DE_CRCO_F$Cbp_Co)) - abs(median(S_DE_CRCO_F$Cbp_Cg)), # Flower CRCO DEs
#                   median(S_DE_CRCO_F$Cbp_Co),
#                   median(S_DE_CRCO_F$Cbp_Cg),
#                   abs(median(S_DE_CGCR_F$Cbp_Co)) - abs(median(S_DE_CGCR_F$Cbp_Cg)), # Flower CGCR DEs
#                   median(S_DE_CGCR_F$Cbp_Co),
#                   median(S_DE_CGCR_F$Cbp_Cg),
#                   abs(median(S_DE_CGCO_F$Cbp_Co)) - abs(median(S_DE_CGCO_F$Cbp_Cg)), # Flower CGCO DEs
#                   median(S_DE_CGCO_F$Cbp_Co),
#                   median(S_DE_CGCO_F$Cbp_Cg),
#                   abs(median(S_L$Cbp_Co)) - abs(median(S_L$Cbp_Cg)), # Leaf all
#                   median(S_L$Cbp_Co),
#                   median(S_L$Cbp_Cg),
#                   abs(median(S_DE_CRCO_L$Cbp_Co)) - abs(median(S_DE_CRCO_L$Cbp_Cg)), # Leaf CRCO DEs
#                   median(S_DE_CRCO_L$Cbp_Co),
#                   median(S_DE_CRCO_L$Cbp_Cg),
#                   abs(median(S_DE_CGCR_L$Cbp_Co)) - abs(median(S_DE_CGCR_L$Cbp_Cg)), # Leaf CGCR DEs
#                   median(S_DE_CGCR_L$Cbp_Co),
#                   median(S_DE_CGCR_L$Cbp_Cg),
#                   abs(median(S_DE_CGCO_L$Cbp_Co)) - abs(median(S_DE_CGCO_L$Cbp_Cg)), # Leaf CGCO DEs
#                   median(S_DE_CGCO_L$Cbp_Co),
#                   median(S_DE_CGCO_L$Cbp_Cg),
#                   abs(median(S_R$Cbp_Co)) - abs(median(S_R$Cbp_Cg)), # Root all
#                   median(S_R$Cbp_Co),
#                   median(S_R$Cbp_Cg),
#                   abs(median(S_DE_CRCO_R$Cbp_Co)) - abs(median(S_DE_CRCO_R$Cbp_Cg)), # Root CRCO DEs
#                   median(S_DE_CRCO_R$Cbp_Co),
#                   median(S_DE_CRCO_R$Cbp_Cg),
#                   abs(median(S_DE_CGCR_R$Cbp_Co)) - abs(median(S_DE_CGCR_R$Cbp_Cg)), # Root CGCR DEs
#                   median(S_DE_CGCR_R$Cbp_Co),
#                   median(S_DE_CGCR_R$Cbp_Cg),
#                   abs(median(S_DE_CGCO_R$Cbp_Co)) - abs(median(S_DE_CGCO_R$Cbp_Cg)), # Root CGCO DEs
#                   median(S_DE_CGCO_R$Cbp_Co),
#                   median(S_DE_CGCO_R$Cbp_Cg)
# )

CRCO_S_value <- c(median(((S_F$Cbp_Co)) + ((S_F$Cbp_Cg))), # Flower all
                  median(S_F$Cbp_Co),
                  median(S_F$Cbp_Cg),
                  ...
                  median(S_DE_CRCO_R$Cbp_Co),
                  median(S_DE_CRCO_R$Cbp_Cg),
                  median(((S_DE_CGCR_R$Cbp_Co)) + ((S_DE_CGCR_R$Cbp_Cg))), # Root CGCR DEs
                  median(S_DE_CGCR_R$Cbp_Co),
                  median(S_DE_CGCR_R$Cbp_Cg),
                  median(((S_DE_CGCO_R$Cbp_Co)) + ((S_DE_CGCO_R$Cbp_Cg))), # Root CGCO DEs
                  median(S_DE_CGCO_R$Cbp_Co),
                  median(S_DE_CGCO_R$Cbp_Cg)
)
# Combine all vectors to a dataframe
CRCO_S_plot <- cbind(CRCO_S_case, CRCO_S_index, CRCO_S_tissue, CRCO_S_value)
# change the format of the dataframe
CRCO_S_plot <- print.data.frame(data.frame(CRCO_S_plot), quote=FALSE)
str(CRCO_S_plot)
CRCO_S_plot$CRCO_S_index <- factor(CRCO_S_plot$CRCO_S_index, levels = c("S_Co","S_Cg","delta_S"))
CRCO_S_plot$CRCO_S_case <- factor(CRCO_S_plot$CRCO_S_case, levels = c("All","Phylogeny","MST","Phylogeny + MST"))
CRCO_S_plot$CRCO_S_tissue <- factor(CRCO_S_plot$CRCO_S_tissue, levels = c("Flower","Leaf","Root"))
CRCO_S_plot$CRCO_S_value <- as.numeric(CRCO_S_plot$CRCO_S_value)
str(CRCO_S_plot)
CRCO_S_plot$Comparison <- "CR2CO"
names(CRCO_S_plot) <- c("Case","Index","Tissue","Value","Comparison")
getwd()
write.table(CRCO_S_plot, file = "OutputData/ggplot2PlotData for S indecies in CRCO.txt", col.names = T, row.names = F, sep = "\t")

# Plot
library(ggplot2)
ggplot(CRCO_S_plot, aes(x=Tissue, y=Value)) +
  geom_hline(yintercept=0, linetype="dashed", color = "grey31") +
  geom_point(aes(fill=Index, shape=Index),size = 2, stroke = 1) + # stroke: making symbol bold
