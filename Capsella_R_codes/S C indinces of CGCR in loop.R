...
Capsella_Diploids_ReadsCount_TMM_Filtered.txt
...
for (i in tissue) {
  ...
  S$Ave_CGCR <- rowMeans(S[,c("mean_CG","mean_CR")])
  ...
  S$Cbp_Cg <- (S$mean_Cbp_Cg - S$Ave_CGCR) / S$Ave_CGCR  # Subgenome Cg 
  S$ASI_Cg <- (S$mean_ASI_Cg - S$Ave_CGCR) / S$Ave_CGCR
  S$EUR_Cg <- (S$mean_EUR_Cg - S$Ave_CGCR) / S$Ave_CGCR
  S$ME_Cg <- (S$mean_ME_Cg - S$Ave_CGCR) / S$Ave_CGCR
  S$CASI_Cg <- (S$mean_CASI_Cg - S$Ave_CGCR) / S$Ave_CGCR
  
  S$Cbp_Co <- (S$mean_Cbp_Co - S$Ave_CGCR) / S$Ave_CGCR  # Subgenome Co
  S$ASI_Co <- (S$mean_ASI_Co - S$Ave_CGCR) / S$Ave_CGCR
  S$EUR_Co <- (S$mean_EUR_Co - S$Ave_CGCR) / S$Ave_CGCR
  S$ME_Co <- (S$mean_ME_Co - S$Ave_CGCR) / S$Ave_CGCR
  S$CASI_Co <- (S$mean_CASI_Co - S$Ave_CGCR) / S$Ave_CGCR
  ...
  for (com in c("CGCO", "CGCR", "CRCO")){
    ...
    write.table(S, file = paste0("OutputData/CGCR_S_All_",Tissue,".txt"), quote = F, sep = "\t", row.names = F)
    
    assign(paste0("S_DE_",com,"_",i), S_DE)
    write.table(S_DE, file = paste0("OutputData/CGCR_S_",com,"_DEs_",Tissue,".txt"), quote = F, sep = "\t", row.names = F)
    ...
    write.table(S_DE, file = paste0("OutputData/CGCR_SC_",com,"_DEs_",Tissue,".txt"), quote = F, sep = "\t", row.names = F)
...
CGCR_S_case <- rep(c(rep("All",3), rep("Phylogeny",3), rep("MST", 3), rep("Phylogeny + MST",3)),3) # 3 tissues
CGCR_S_index <- c(rep(c("delta_S","S_Co", "S_Cg"), 12)) # 4 cases, 3 tissues (F,L,R)
CGCR_S_tissue <- c(rep("Flower", 12), rep("Leaf", 12), rep("Root", 12))
CGCR_S_value <- c(median((S_F$Cbp_Co) + (S_F$Cbp_Cg)), # Flower all
                  median(S_F$Cbp_Co),
                  median(S_F$Cbp_Cg),
...
CGCR_S_plot <- cbind(CGCR_S_case, CGCR_S_index, CGCR_S_tissue, CGCR_S_value)
# change the format of the dataframe
CGCR_S_plot <- print.data.frame(data.frame(CGCR_S_plot), quote=FALSE)
str(CGCR_S_plot)
CGCR_S_plot$CGCR_S_index <- factor(CGCR_S_plot$CGCR_S_index, levels = c("S_Co","S_Cg","delta_S"))
CGCR_S_plot$CGCR_S_case <- factor(CGCR_S_plot$CGCR_S_case, levels = c("All","Phylogeny","MST","Phylogeny + MST"))
CGCR_S_plot$CGCR_S_tissue <- factor(CGCR_S_plot$CGCR_S_tissue, levels = c("Flower","Leaf","Root"))
CGCR_S_plot$CGCR_S_value <- as.numeric(CGCR_S_plot$CGCR_S_value)
str(CGCR_S_plot)
CGCR_S_plot$Comparison <- "CG2CR"
names(CGCR_S_plot) <- c("Case","Index","Tissue","Value","Comparison")

write.table(CGCR_S_plot, file = "OutputData/ggplot2PlotData for S indecies in CGCR.txt", col.names = T, row.names = F, sep = "\t")
...
# Plot
library(ggplot2)
ggplot(CGCR_S_plot, aes(x=Tissue, y=Value)) +
